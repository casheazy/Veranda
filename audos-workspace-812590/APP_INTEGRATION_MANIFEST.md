...
| Seed or backfill rows into a WorkspaceDB table **as an agent** | `server-functions` (one-shot hook — see the `workspace-db` entry below) |
...
**Seeding or backfilling rows as an agent (not from the app).** There is no MCP tool that writes data: `workspace_db_query` / `workspace_db_raw_query` are read-only and `workspace_db_create_table` only creates schema. The `window.__workspaceDb` insert methods run only in a visitor's browser, and the REST insert above requires an `X-Workspace-DB-Token` that agent VMs are not issued. Use a one-shot `server-functions` hook instead — the hooks API is reachable from an agent VM without auth:

1. `POST https://audos.com/api/workspaces/812590/hooks` with a `code` body that calls `db.insert('<table>', rows)`.
2. `POST https://audos.com/api/workspaces/812590/hooks/{hookName}/execute` once; check `_meta.success` / `_meta.error` in the response.
3. Verify the rows with `workspace_db_query`.
4. `DELETE https://audos.com/api/workspaces/812590/hooks/{hookId}` — leaving the hook behind leaves a public write path into the table.

Two gotchas, both verified against this workspace: `json` columns must be passed as `JSON.stringify(value)` (a raw array fails with `Insert failed: invalid input syntax for type json`, and `db.update` fails the same way), and rows written by a hook or by MCP have `session_id = NULL`, so apps must read them with `{ shared: true }`.
