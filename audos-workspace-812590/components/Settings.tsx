...
import { SUBSCRIPTION_PRICE_NGN, SUBSCRIPTION_PRICE_USD_CENTS } from '../apps/Veranda/types';
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/payments/subscribe', {
        ...
        body: JSON.stringify({
          // Veranda plan — keep in sync with apps/Veranda/types.ts
                    ...
                    {paymentConfig && (
                      <div className="p-4 bg-gray-50 rounded-xl">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-medium text-gray-900">
                            Veranda Monthly — unlimited reports
                          </span>
                          <span className="text-lg font-bold text-gray-900">
                            ₦{SUBSCRIPTION_PRICE_NGN.toLocaleString()}/mo
                            <span className="block text-right text-[10px] font-normal text-gray-500">
                              charged as {formatPrice(SUBSCRIPTION_PRICE_USD_CENTS)} USD
                            </span>
                          </span>
                        </div>
